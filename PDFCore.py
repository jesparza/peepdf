...
class PDFObject:
    ...
    def replace(self, string1, string2):
        ...
        '''
        if self.value.find(
                string1) == -1 and self.rawValue.find(string1) == -1:
    ...
    def update(self, decrypt=False):
        ...
        if isJavascript(self.value):
            ...
            self.JSCode, self.unescapedBytes, self.urlsFound, jsErrors, jsContexts[
                'global'] = analyseJS(self.value, jsContexts['global'], isManualAnalysis)
            if jsErrors != []:
...
class PDFReference (PDFObject):
    ...
    def __init__(self, id, genNumber='0'):
        ...
        self.compressedIn = None
        self.encrypted = False
        self.value = self.rawValue = self.encryptedValue = id + \
            ' ' + genNumber + ' R'
        self.id = id
...
class PDFArray (PDFObject):
            ...
            if element is not None:
                ...
                if element.isFaulty():
                    for error in valueObject.getErrors():
                        self.addError(
                            'Children element contains errors: ' +
                            error)
                if type in [
                        'string',
                        'hexstring',
                        'array',
                        'dictionary'] and self.encrypted and not decrypt:
    ...
    def getElementByName(self, name):
        ...
        for element in self.elements:
            if element is not None:
                if element.getType(
                ) == 'dictionary' or element.getType() == 'array':
...
class PDFDictionary (PDFObject):
    ...
    def update(self, decrypt=False):
        ...
        for i in range(len(keys)):
            if values[i] == None:
                errorMessage = 'Non-existing value for key "' + \
                    str(keys[i]) + '"'
                if isForceMode:
                    self.addError(errorMessage)
            ...
            if type in [
                ...
                if ret[0] == -1:
                    errorMessage = 'Error encrypting element'
                    self.addError(errorMessage)
            self.encryptedValue += rawValue + ' ' + \
                str(valueObject.getEncryptedValue()) + newLine
            self.rawValue += rawValue + ' ' + \
                str(valueObject.getRawValue()) + newLine
...
class PDFStream (PDFDictionary):
    ...
    def update(self, onlyElements=False, decrypt=False, algorithm='RC4'):
        ...
        for i in range(len(keys)):
            ...
            if valueElement.containsJS():
                ...
                self.urlsFound = list(
                    set(self.urlsFound + valueElement.getURLs()))
            ...
            if type in [
                ...
                if ret[0] == -1:
                    ...
                    self.addError(errorMessage)
            self.encryptedValue += rawValue + ' ' + \
                str(valueElement.getEncryptedValue()) + newLine
            self.rawValue += rawValue + ' ' + \
                str(valueElement.getRawValue()) + newLine
            self.value += keys[i] + ' ' + v + newLine
        self.encryptedValue = self.encryptedValue[:-1] + ' >>'
        self.rawValue = self.rawValue[:-1] + ' >>'
        self.value = self.value[:-1] + ' >>'
        ...
        if not onlyElements:
            ...
            if self.deletedFilters or self.newFilters or self.modifiedStream or self.modifiedRawStream or self.encrypted:
                if self.deletedFilters:
                    ...
                    if len(
                            self.encodedStream) > 0 or len(
                            self.decodedStream) > 0:
                        self.cleanStream()
                                    ...
                                    try:
                                        if algorithm == 'RC4':
                                            ...
                                            if ret[0] != -1:
                                                ...
                                                errorMessage = 'AES decryption error: ' + \
                                                    ret[1]
                                                if isForceMode:
                                                    self.addError(errorMessage)
                                                else:
                                        ...
                                        errorMessage = 'Error decrypting stream with ' + \
                                            str(algorithm)
                                        if isForceMode:
                                            self.addError(errorMessage)
                                        else:
                    ...
                    for i in range(len(filterElements)):
                        ...
                        if filter.getType() == 'name':
                            ...
                            if isForceMode:
                                ...
                                return (-1,
                                        'One of the filters type is not valid')
                else:
                    if isForceMode:
    ...
    def getEncryptedValue(self):
        return self.encryptedValue + newLine + 'stream' + \
            newLine + self.rawStream + newLine + 'endstream'
    ...
    def getRawValue(self):
        if self.isEncoded():
            ...
            stream = self.decodedStream
        return self.rawValue + newLine + 'stream' + \
            newLine + stream + newLine + 'endstream'

    def getValue(self):
        return self.value + newLine + 'stream' + newLine + \
            self.decodedStream + newLine + 'endstream'
    ...
    def replace(self, string1, string2):
        ...
        if not self.modifiedRawStream:
            ...
            if self.decodedStream.find(string1) != -1:
                self.decodedStream = self.decodedStream.replace(
                    string1,
                    string2)
                stringFound = True
                if errorMessage == 'String not found':
...
class PDFObjectStream (PDFStream):
    ...
    def update(
        ...
        if '/First' in self.elements and self.elements['/First'] != None:
            ...
            if isForceMode:
                self.addError(
                    'No /First element in the object stream or it\'s None')
            else:
                return (-1,
                        'No /First element in the object stream or it\'s None')
        if '/N' in self.elements and self.elements['/N'] != None:
            ...
            if isForceMode:
                self.addError(
                    'No /N element in the object stream or it\'s None')
            else:
                ...
                if self.deletedFilters:
                    ...
                    if self.modifiedStream or self.modifiedRawStream:
                        ...
                        if numbers != [] and len(numbers) % 2 == 0:
                            ...
                            if isForceMode:
                                self.addError(
                                    'Missing offsets in object stream')
                            else:
                        ...
                        for objectId in self.indexes:
                            ...
                            tmpStreamObjectsInfo += str(objectId) + \
                                ' ' + str(offset) + ' '
                            object = self.compressedObjectsDict[objectId][1]
                            tmpStreamObjects += object.toFile()
                            self.compressedObjectsDict[objectId] = [
                                offset,
                                object]
                        self.decodedStream = tmpStreamObjectsInfo + \
                            tmpStreamObjects
                        self.firstObjectOffset = len(tmpStreamObjectsInfo)
                        self.setElementValue(
                            '/First', str(self.firstObjectOffset))
                        self.numCompressedObjects = len(
                            self.compressedObjectsDict)
                        self.setElementValue(
                            '/N', str(self.numCompressedObjects))
                        if self.isEncodedStream:
                            self.encode()
                            self.size = len(self.encodedStream)
                        else:
...
class PDFIndirectObject:
    ...
    def toFile(self):
        ...
        output = str(self.id) + ' ' + str(self.generationNumber) + \
            ' obj' + newLine + rawValue + newLine + 'endobj' + newLine * 2
    ...
    def addEntry(self, objectId, newEntry):
        ...
        for i in range(len(self.subsections)):
            ...
            if ret[0] != -1:
                ...
                errorMessage = ret[1]
                self.addError(errorMessage)
            if subsection.getFirstObject() + \
                    subsection.getNumObjects() < objectId:
                prevSubsection = i
        else:
            try:
...
class PDFBody:
                    ...
                    if typeObject is None:
                        ...
                        if type == '/XRef':
                            ...
                            for compressedId in compressedObjectsDict:
                                ...
                                compressedObject = compressedObjectsDict[
                                    compressedId][1]
                                self.setObject(
                                    compressedId,
                                    compressedObject,
    ...
    def setObject(self, id=None, object=None, offset=None, modification=False):
        ...
        if id in self.objects:
            ...
            pdfIndirectObject.setOffset(offset)
            size = 12 + 3 * \
                len(newLine) + len(str(object.getRawValue())) + len(str(id))
    ...
    def updateStats(self, id, pdfObject, delete=False):
        ...
        for action in monitorizedActions:
            ...
            if index != -1 and (action == '/JS ' or len(value) == index + len(
                    action) or value[index + len(action)] in delimiterChars + spacesChars):
        ...
        for element in monitorizedElements:
            ...
            if index != -1 and (element == '/EmbeddedFiles ' or len(value) == index + len(
                    element) or value[index + len(element)] in delimiterChars + spacesChars):
        ...
        for rawIndicatorVar in monitorizedIndicators['versionBased'].keys():
            indicatorType = monitorizedIndicators[
                'versionBased'][rawIndicatorVar][1]
            ...
            if indicatorVar not in (None, False) or delete:
                printedIndicator = monitorizedIndicators[
                    'versionBased'][rawIndicatorVar][0]
                if printedIndicator in self.suspiciousElements:
                    if delete:
                        if id in self.suspiciousElements[printedIndicator]:
                            self.suspiciousElements[
                                printedIndicator].remove(id)
                    ...
                    for jsCode in jsCodeArray:
                        ...
                        for vuln in jsVulns:
                            if jsCode.find(vuln) != -1:
                                if vuln in self.vulns and id in self.vulns[
                                        vuln]:
        ...
        if objectType == 'stream':
            ...
            if len(streamContent) > 327 and streamContent[
                    236:240] == 'SING' and streamContent[327] != '\0':
                # CVE-2010-2883
...
class PDFTrailer:
    ...
    def getStats(self):
        ...
        if self.dict.hasElement(
                '/ID') and self.id is not None and self.id != '' and self.id != ' ':
            stats['ID'] = self.id
        else:
            stats['ID'] = None
...
class PDFFile:
    ...
    def addCrossRefTableSection(self, newSectionArray):
        if newSectionArray is not None and isinstance(
            newSectionArray,
            list) and len(newSectionArray) == 2 and (
            newSectionArray[0] == None or isinstance(
                newSectionArray[0],
                PDFCrossRefSection)) and (
                newSectionArray[1] == None or isinstance(
                    newSectionArray[1],
                    PDFCrossRefSection)):
    ...
    def addTrailer(self, newTrailerArray):
        if newTrailerArray is not None and isinstance(
            newTrailerArray,
            list) and len(newTrailerArray) == 2 and (
            newTrailerArray[0] == None or isinstance(
                newTrailerArray[0],
                PDFTrailer)) and (
                newTrailerArray[1] == None or isinstance(
                    newTrailerArray[1],
                    PDFTrailer)):
    ...
    def calculateScore(self, checkOnVT=False):
        ...
        if self.numObjects < 30:
            threshold_score = (
                1.0 - (30.0 - self.numObjects) / 100.0) * MAX_THRESHOLD_SCORE
        else:
        ...
        for indicator in indicators:
            ...
            if indicator in ('CreatorList', 'ProducerList'):
                ...
                for builder in indicatorVal:
                    if builder is None:
                        continue
                    builderKey = get_close_matches(
                        builder,
                        PDFBuildersScore.keys(),
                        n=1,
                        cutoff=0.6)
                    if builderKey != []:
            ...
            if isinstance(
                    scoreVal, (tuple, list)) and isinstance(
                    indicatorVal, (list, tuple)):
                limit = scoreVal[1]
        ...
        if filterScore > 0:
            scoringCard.append(
                ('streams present with 1 filter (%d)' %
                 singleFilter,
                 filterScore))
        ...
        if obfuscationScore > 0:
            scoringCard.append(
                ('Obfuscated Javascript (%d)' %
                 obfuscatedStreamCount,
                 obfuscationScore))
    ...
    def createObjectStream(self, version=None, id=None, objectIds=[]):
        ...
        for compressedId in objectIds:
            ...
            if object is None:
                errorMessage = 'Object ' + \
                    str(compressedId) + ' cannot be compressed: it does not exist'
                if isForceMode:
                    self.addError(errorMessage)
                    numObjects -= 1
                else:
                ...
                if objectType == 'stream':
                    ...
                    if objectType == 'dictionary' and object.hasElement(
                            '/U') and object.hasElement('/O') and object.hasElement('/R'):
                        errorMessage = 'Encryption dictionaries cannot be compressed'
                        self.addError(errorMessage)
                        numObjects -= 1
                    object.setCompressedIn(id)
                    offset = len(tmpStreamObjects)
                    tmpStreamObjectsInfo += str(compressedId) + \
                        ' ' + str(offset) + ' '
                    tmpStreamObjects += object.toFile()
        ...
        compressedDict = {'/Type': PDFName('ObjStm'),
    ...
    def createXrefStream(self, version, id=None):
        ...
        if ret[0] == -1:
            ...
            self.addError(ret[1])
        try:
            trailerStream = PDFTrailer(
                PDFDictionary(
                    elements=elementsTrailerDict))
        except Exception as e:
        ...
        try:
            trailerSection = PDFTrailer(
                PDFDictionary(
                    elements=dict(elementsTrailerDict)))  # PDFDictionary())
        except Exception as e:
            errorMessage = 'Error creating PDFTrailer'
            if e.message != '':
                errorMessage += ': ' + e.message
            ...
            if indirectObject is not None:
                ...
                if object is not None:
                    if object.isCompressed():
                        ...
                        objectStream = self.body[
                            version].getObject(objectStreamId)
    ...
    def decrypt(self, password=''):
        ...
        if '/V' in encDict:
            ...
            if algVersion is not None and algVersion.getType() == 'integer':
                ...
                if algVersion == 4 or algVersion == 5:
                    ...
                    if '/CF' in encDict:
                        cfDict = encDict['/CF']
                        if cfDict is not None and cfDict.getType(
                        ) == 'dictionary':
                            cfDict = cfDict.getElements()
                            for cryptFilter in cfDict:
                                cryptFilterDict = cfDict[cryptFilter]
                                if cryptFilterDict is not None and cryptFilterDict.getType(
                                ) == 'dictionary':
                                    ...
                                    if '/CFM' in cryptFilterDict:
                                        ...
                                        if cfmValue is not None and cfmValue.getType(
                                        ) == 'name':
                                            ...
                                            if cfmValue == 'None':
                                                ...
                                                algorithms[
                                                    cryptFilter].append('RC4')
                                            elif cfmValue == '/AESV2':
                                                algorithms[
                                                    cryptFilter].append('AES')
                                    ...
                                    if '/Length' in cryptFilterDict and cfmValue != '/AESV3':
                                        ...
                                        if keyLength is not None and keyLength.getType(
                                        ) == 'integer':
                                            ...
                                            # Check if the length element
                                            # contains bytes instead of bits as
                                            # usual
                                            if keyLength < 40:
                                                keyLength *= 8
                                        else:
        ...
        if '/O' in encDict:
            dictO = encDict['/O']
            if dictO is not None and dictO.getType() in [
                    'string',
                    'hexstring']:
        ...
        if '/OE' in encDict:
            dictOE = encDict['/OE']
            if dictOE is not None and dictOE.getType() in [
                    'string',
                    'hexstring']:
        ...
        if '/U' in encDict:
            dictU = encDict['/U']
            if dictU is not None and dictU.getType() in [
                    'string',
                    'hexstring']:
        ...
        if '/UE' in encDict:
            dictUE = encDict['/UE']
            if dictUE is not None and dictUE.getType() in [
                    'string',
                    'hexstring']:
        ...
        if '/EncryptMetadata' in encDict:
            ...
            if encryptMetadata is not None and encryptMetadata.getType(
            ) == 'bool':
                ...
                for id in indirectObjectsIds:
                    ...
                    if indirectObject is not None:
                        ...
                        if object is not None and not object.isCompressed():
                            objectType = object.getType()
                            if objectType in [
                                'string', 'hexstring', 'array', 'dictionary'] or (
                                objectType == 'stream' and (
                                    object.getElement('/Type') == None or (
                                        object.getElement('/Type').getValue() not in [
                                            '/XRef', '/Metadata'] or (
                                ...
                                if objectType in [
                                    ...
                                    if object.getElement(
                                            '/Type') != None and object.getElement('/Type').getValue() == '/EmbeddedFile':
                                        if revision < 5:
                                            ret = computeObjectKey(
                                                id,
                                                generationNum,
    ...
    def detectGarbageBetweenObjects(self):
            ...
            for element in content.keys():
                ...
                if isinstance(offset, tuple):
                    ...
                    for object in offset:
                        offsetList.append(
                            (version, int(
                                object[0]), object[1], object[1] + object[2]))
        ...
        for obj in garbageList:
            if bytesText in self.body[obj[0]].suspiciousElements:
                if obj[1] not in self.body[
                        obj[0]].suspiciousElements[bytesText]:
                    self.body[
                        obj[0]].suspiciousElements[bytesText].append(
                        obj[1])
            else:
        ...
        for obj in spaceGapList:
            if gapText in self.body[obj[0]].suspiciousElements:
                if obj[1] not in self.body[obj[0]].suspiciousElements[gapText]:
                    self.body[
                        obj[0]].suspiciousElements[gapText].append(
                        obj[1])
            else:
    ...
    def getBasicMetadata(self, version):
        ...
        if 'producer' not in basicMetadata:
            ...
            if ids is not None and ids != []:
                for id in ids:
                    producer = self.getMetadataElement(
                        id,
                        version,
                        'pdf:Producer')
        ...
        if version is None:
            ...
            if infoId is not None:
                ...
                if infoObject is None and version == 0 and self.getLinearized():
                    # Linearized documents can store Info object in the next
                    # update
        ...
        for version in range(self.updates + 1):
            ...
            if self.linearized:
                ...
                for objectId in isolatedList:
                    ...
                    if object.getType() in objectTypeList and object.hasElement(
                            '/Linearized'):
                        continue
        ...
        if version is None:
            for i in range(self.updates + 1):
                matchedObjects.append(
                    self.body[i].getObjectsByString(toSearch))
            return matchedObjects
        else:
            if version > self.updates or version < 0:
                return None
            ...
            if objectStreams != []:
                for objStream in objectStreams:
                    if objStream in indirectObjects:
                        ...
                        if indirectObject is not None:
                            objectStreamsOffsets[
                                objStream] = indirectObject.getOffset()
            if version == 0:
                ...
                if indirectObject is not None:
                    ...
                    if object is not None and object.isCompressed():
                        ...
                        if compressedIn in objectStreamsOffsets:
                            objectOffset = objectStreamsOffsets[
                                compressedIn] + objectOffset + 20
    ...
    def getScoringFactors(self, checkOnVT=False, nonNull=False):
        ...
        for version in range(self.updates + 1):
            ...
            for stream in streams:
                ...
                if isinstance(streamFilter, list):
                    streamDict['numFilters'] = len(streamFilter)
                elif isinstance(streamFilter, str):
    ...
    def getStats(self):
        ...
        for version in range(self.updates + 1):
            ...
            objectsById = sorted(
                self.body[version].getObjectsIds(),
                key=lambda x: int(x))
            statsVersion['Objects'] = [
                str(self.body[version].getNumObjects()), objectsById]
            if self.body[version].containsCompressedObjects():
                compressedObjects = self.body[version].getCompressedObjects()
                statsVersion['Compressed Objects'] = [
                    str(len(compressedObjects)), compressedObjects]
            else:
            ...
            if len(containingJS) > 0:
                statsVersion['Objects with JS code'] = [
                    str(len(containingJS)), containingJS]
            else:
    ...
    def makePDF(self, pdfType, content):
        ...
        catalogSize = staticIndirectObjectSize + \
            len(catalogDictionary.getRawValue())
        ...
        pagesDictionary = PDFDictionary(
            elements={
                ...
                '/Count': PDFNum('1')})
        pagesSize = len(
            pagesDictionary.getRawValue()) + staticIndirectObjectSize
        ...
        if pdfType == 'open_action_js':
            ...
            actionDictionary = PDFDictionary(
                elements={
                    ...
                    '/JS': PDFReference('5')})
            actionSize = len(
                actionDictionary.getRawValue()) + staticIndirectObjectSize
        ...
        try:
            ...
            for v in range(version + 1):
                        ...
                        if object is not None:
                            ...
                            if not object.isCompressed():
                                ...
                                if numSubSectionsInXrefStream != 0:
                                    ret = streamSection.updateOffset(
                                        id,
                                        offset)
                                    if ret[0] == -1:
                ...
                if xrefStreamObject is not None:
                    ...
                    if streamTrailer is not None:
                        ...
                        if prevXrefStreamOffset != 0:
                            streamTrailer.setPrevCrossRefSection(
                                prevXrefStreamOffset)
                    ...
                    offset = len(outputFileContent)
                    xrefStreamObject.setSize(
                        offset -
                        xrefStreamObject.getOffset())
                ...
                if trailer is not None:
                    ...
                    trailer.setOffset(offset)
                    if trailer.getCatalogId(
                    ) != None and trailer.getSize() != 0:
                        trailer.setNumObjects(maxId + 1)
                        if prevXrefSectionOffset != 0:
                            trailer.setPrevCrossRefSection(
                                prevXrefSectionOffset)
    ...
    def verifyXrefOffsets(self):
        ...
        for version in range(self.updates + 1):
            ...
            if filter(None, xrefSection) == []:
                self.body[version].suspiciousProperties.append(
                    'Xref Table missing')
                continue
            for section in xrefSection:
                ...
                for subsection in section.getSubsectionsArray():
                    ...
                    for count, objectEntry in enumerate(
                        ...
                        if (
                            objectId not in realObjectOffsets.keys() and not self.linearized) or (
                            objectId in realObjectOffsets.keys() and abs(
                                realObjectOffsets[objectId] -
                                objectOffset) > 4):
...
class PDFParser:
    ...
    def parse(
        ...
        matchVersion = re.findall(
            '%(PDF-|!PS-Adobe-\d{1,2}\.\d{1,2}\sPDF-)(\d{1,2}\.\d{1,2})',
            versionLine)
        if matchVersion == []:
            if forceMode:
        ...
        if len(binaryLine) > 3:
            ...
            if binaryLine[0] == '%' and ord(
                binaryLine[1]) >= 128 and ord(
                binaryLine[2]) >= 128 and ord(
                binaryLine[3]) >= 128 and ord(
                    binaryLine[4]) >= 128:
                pdfFile.binary = True
        ...
        # Getting the body, cross reference table and trailer of each part of
        # the file
        for i in range(len(self.fileParts)):
            ...
            body = PDFBody()
            rawIndirectObjects = self.getIndirectObjects(
                bodyContent,
                looseMode)
            if rawIndirectObjects != []:
                for j in range(len(rawIndirectObjects)):
                    ...
                    self.tempVarObfuscation = False
                    ret = self.createPDFIndirectObject(
                        rawObject,
                        forceMode,
                        looseMode)
                    if ret[0] != -1:
                        ...
                        if not forceMode:
                            ...
                            pdfFile.addError(
                                'Error parsing object: ' + str(objectHeader) + ' (' + str(ret[1]) + ')')
            else:
            ...
            if body.containsXrefStreams():
                ret = self.createPDFTrailerFromStream(
                    xrefObject,
                    trailerContent)
                if ret[0] != -1:
                    streamTrailer = ret[1]
                ...
                if ret[0] != -1 and not pdfFile.isEncrypted():
                    ...
                    if encryptDict is not None:
                        pdfFile.setEncrypted(True)
                    fileId = trailer.getDictEntry('/ID')
            if pdfFile.getEncryptDict() == None and encryptDict is not None:
                ...
                if objectType == 'dictionary':
                    pdfFile.setEncryptDict(
                        [encryptDictId, encryptDict.getElements()])
                        ...
                        if isForceMode:
                            if pdfCrossRefSubSection is not None:
                                pdfCrossRefSubSection.addError(
                                    'Bad format for cross reference entry: ' +
                                    line)
                            else:
                                pdfCrossRefSubSection = PDFCrossRefSubSection(
                                    0,
        ...
        if objectStream is not None:
            ...
            if xrefObject is not None:
                if xrefObject.hasElement('/Size'):
                    ...
                    if sizeObject is not None and sizeObject.getType(
                    ) == 'integer':
                ...
                if xrefObject.hasElement('/Index'):
                    subsectionIndexesObject = xrefObject.getElementByName(
                        '/Index')
            ...
            if xrefStreamObject is not None:
                for element in trailerElements:
                    if xrefStreamObject.hasElement(element):
                        dict[element] = xrefStreamObject.getElementByName(
                            element)
                try:
    ...
    def getIndirectObjects(self, content, looseMode=False):
        ...
        if not looseMode:
            regExp = re.compile(
                '((\d{1,10}\s\d{1,10}\sobj).*?endobj)',
                re.DOTALL)
            matchingObjects = regExp.findall(content)
        else:
            regExp = re.compile(
                '((\d{1,10}\s\d{1,10}\sobj).*?)\s\d{1,10}\s\d{1,10}\sobj',
                re.DOTALL)
            ...
            while matchingObjectsAux != []:
                if matchingObjectsAux[0] != []:
                    ...
                    content = content[
                        content.find(objectBody) +
                        len(objectBody):]
                    matchingObjectsAux = regExp.findall(content)
                else:
                    matchingObjectsAux = []
            lastObject = re.findall(
                '(\d{1,5}\s\d{1,5}\sobj)',
                content,
                re.DOTALL)
            if lastObject != []:
    ...
    def readObject(
        ...
        if objectType is not None:
            objectsTypeArray = [self.delimiters[i][2]
            ...
            if index != -1:
                ...
                if isForceMode:
                    pdfFile.addError(
                        'Unknown object type while parsing object')
        ...
        if numClosingDelims == 0:
            ...
            while indexChar != len(newContent):
                ...
                if char == delim[1] or (char + nextChar) == delim[1]:
                    if char != ')' or indexChar == 0 or newContent[
                            indexChar -
                            1] != '\\':
                        return (0, output)
                    else:
